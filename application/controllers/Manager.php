...
class Manager extends CI_Controller {
    ...
    public function index(){
        redirect(base_url('manager/daftar_total'));
    }
    ...
    public function review(){
        ...
        $this->header();
        $this->log();
        $this->load->view('manager/review', $data);
        $this->load->view('layout/footer');
    }
    ...
    public function approve(){
        ...
        $round = $this->M_Log->get_round_log(array('id_surat'=>"$id",'next_user'=>5))->row()->round;
        if($ttd=="manager"){
            $status=7;
        }
        else{
